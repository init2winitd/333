...
The disposable production-shaped alpha is deployed from an exact commit with `scripts\deploy-alpha.ps1 -Commit <hash>`. It uses the dedicated worktree and three roots under `C:\alpha`, listens on port 7337, applies the existing `agentb-svc` ACL policy without creating another account or firewall rule, stops only an executable running from the alpha application root, and refuses a dirty alpha worktree. Run it from a normal PowerShell window and approve the same-user installer UAC prompt. UAC consent cannot be unattended; the first alpha use also requires entering the service-account password and model credentials in alpha Settings because DPAPI secrets are neither copied from production nor written by the deploy script. Every build reports its commit through `/api/state`; a dirty source build is visibly suffixed `+dirty` in the Console.
...
For a source checkout, the first start copies `harness.example.json` to the ignored local `harness.json`; the checkout remains both the application and data root for development. Under **Connections**, set each profile's `base_url` and `model`, then assign profiles to the `main` and optional `aux` roles. An unset `aux` uses `main`. If an endpoint needs an API key, entering it stores the secret in a named user-scoped DPAPI credential; `harness.json` keeps only its `credential` reference. Select **Save**, then select **Test**. The test records an endpoint-reported context size when available; otherwise enter the documented limit. A profile is not runnable until its own context size is known. Startup never waits on a model probe; a successful explicit test stays labeled `ready`. Settings → Security can create the local service account, verify the identity split, and apply/verify the application, data, workspace, and outbound firewall policies through Windows UAC without running Agent_b itself as Administrator; progress and errors remain visible after refresh. Installed host protection uses the separated roots documented above. Follow the [Windows host hardening](docs/HARDENING.md) sequence. Agent_b refuses to guess a context ceiling.
...
Compaction summaries use the optional `aux` profile when its fully rendered request fits that profile's context window. An unavailable, rejecting, or undersized aux profile falls back to the session's main profile; blank aux preserves the single-main-model path. Console timeline entries identify the serving profile and keep compaction inference input/output tokens separate from the main context-budget measurement.
